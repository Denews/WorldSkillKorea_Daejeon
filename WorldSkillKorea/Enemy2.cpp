...
const float Enemy2::BULLET_INTERVAL = 0.5f;

Enemy2::Enemy2(ID3D11Device* device, Texture* texture, XMFLOAT2 position, SubmarineObserver* observer) :
	GameObject(device, texture, 128, 64),
	m_BulletTimer(new GameTimer()),
	m_IsActive(false),
	m_Bullets(new Bullet*[MAX_BULLET_COUNT]),
	m_BulletTexture(new Texture(device, "./res/Enemy/Bullet_R.dds")),
...
void Enemy2::draw(ID3D11DeviceContext* deviceContext, CXMMATRIX viewOrtho)
{
...
XMFLOAT3 Enemy2::getPosition() const
{
